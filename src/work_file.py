...
def CuttingCounter(received_list: list):
    empty_list = []
...
def CreatingDict(len_dict: int, received_list: list):  
    list_int = []
    dict_text = {}
    received_list = CuttingCounter(received_list)
...
def SortingDict(received_dict: dict):
    list_values = list(received_dict.values())
    list_values = sorted(list_values)
    new_dict = CreatingDict(len(list_values), list_values)
    return new_dict
...
def MoveElement(where: str):
    ...
    dict_file = CreatingDict(len(list_file), list_file)
    dict_planned = CreatingDict(len(list_planned), list_planned)
    ...
    Writing(SortingDict(dict_planned), file_planned, True)
    Writing(SortingDict(dict_file), file, True)
