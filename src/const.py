...
writing(creating_dict(len(text_completed), text_completed), file_completed)
# Записываю из файла planned(sort), в файл в папке "planned.txt"
text_planned = file_planned_sort.readlines()
writing(creating_dict(len(text_planned), text_planned), file_planned)
# Записываю из файла stubs, в файлы 'dropped.txt' и 'liked.txt'
text_stubs = file_stubs.readlines()
writing(creating_dict(len(text_stubs), text_stubs), file_liked)
writing(creating_dict(len(text_stubs), text_stubs), file_dropped)
